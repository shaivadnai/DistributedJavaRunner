...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    // 2 Queues allows for reduced chance of false-positive node failure. Instead of
    // having two threads compete for the same queue despite looking for different
    // messages, the pipelines are kept completely separate
    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingElectionMessages,
            LinkedBlockingQueue<Message> incomingHeartbeatMessages, InetSocketAddress myAddress, int myPort,
            ZooKeeperPeerServer peerServer) throws IOException {
        this.incomingElectionMessages = incomingElectionMessages;
        this.incomingHeartbeatMessages = incomingHeartbeatMessages;
    ...
    public void run() {
        ...
        while (!this.isInterrupted()) {
            try {
                ...
                if (received.getMessageType() == MessageType.GOSSIP) {
                    boolean done = false;
                    while (!done) {
                        done = this.incomingHeartbeatMessages.offer(received);
                    }
                }
                // END SHAI EDITS
                else {
                    ...
                    if (sendLeader(received)) {
                        ...
                        // might've entered election between the two previous lines of code, which would
                        // make leader null, hence must test
                        if (leader != null) {
                            ElectionNotification notification = new ElectionNotification(leader.getProposedLeaderID(),
                                    this.peerServer.getPeerState(), this.peerServer.getServerId(),
    ...
    private void sendElectionReply(byte[] msgContent, InetSocketAddress target) {
        Message msg = new Message(Message.MessageType.ELECTION, msgContent, this.myAddress.getHostString(), this.myPort,
    ...
    /**
     * see if we got an Election LOOKING message while we are in FOLLOWING or
     * LEADING
     * 
     * @param received
     * @return
    ...
    private boolean sendLeader(Message received) {
        ...
        if ((receivedState == ZooKeeperPeerServer.ServerState.LOOKING
                || receivedState == ZooKeeperPeerServer.ServerState.OBSERVER)
    ...
    /**
     * if neither sender nor I am looking, and this is an election message, let it
     * disappear
     * 
     * @param received
     * @return
    ...
    private boolean strayElectionMessage(Message received) {
        ...
        if (receivedNotification.getState() != ZooKeeperPeerServer.ServerState.LOOKING
                && this.peerServer.getPeerState() != ZooKeeperPeerServer.ServerState.LOOKING) {
            return true;
        } else {
            return false;
        }
